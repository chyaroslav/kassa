...
type P struct {
	...
	MaxAdminChatID string // ChatId чата администраторов MAX.
	UserChatID     string // ChatId чата пользователей.
	MaxUserChatID  string // ChatId чата пользователей MAX.
	CompanyName    string
}
...
*/
func sendMAXmsg(chatId string, msg string) error {

	type MessageRequest struct {
		Text string `json:"text"`
	}
	msgReq := MessageRequest{Text: msg}
	jsonData, err := json.Marshal(msgReq)
	chUrl := "https://platform-api.max.ru/messages?chat_id=" + chatId
	req, err := http.NewRequest("POST", chUrl, bytes.NewBuffer(jsonData))

	if err != nil {
		return err
	}
	req.Header.Set("Content-Type", "application/json")
	req.Header.Set("Authorization", "f9LHodD0cOIjBx7ZFX5VYS9MeRfrLJz6UIUqczpUyQVD0XkSxfG3oCV_z0jJYryI6SepByyygX1KQuZwz8UJ")
...
func (k *K) sendMsg(mType int8, a ...any) {
	//mType: 0 - шлем только админам, 1 - шлем всем
	var msg string
	for i := 0; i < len(a); i++ {
		msg = msg + fmt.Sprintf("%v", a[i])
	}
	if mType == 1 {
		sendTGmsg(k.params.UserChatID, msg)
		sendMAXmsg(k.params.MaxUserChatID, msg)
	}
	sendTGmsg(k.params.AdminChatID, msg)
	sendMAXmsg(k.params.MaxAdminChatID, msg)

}
func sendTGmsg(chatId string, msg string) error {
